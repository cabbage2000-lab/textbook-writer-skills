...
| 动笔前先把工作目录规划好（可选，git 版本管理 + 多教材工作区） | `用 textbook-init 初始化教材工作目录` |
...
1 个主 skill 调度 3 个子 skill 走五阶段流程，另有 1 个独立辅助 skill 负责动笔前的工作目录规划。5 个 skill 是一个整体组合（跨 skill 以相对路径互相引用），需整体安装：
...
textbook-init (独立辅助 skill：工作目录初始化)
  └─ 不入调度链：动笔前规划并创建单本目录或多教材工作区（可选 git 与 README 工作说明），产出交给 textbook 开写
```
...
```text
skills/               # 5 个 skill；各自 references/ 存放知识底座（UbD 指南、Bloom 动词表、章节模板、例题验证规范、工作目录布局等）
...
| M4 | 主 skill 中断-续写机制验证（状态机正确定位续点） | [eval 4](evals/evals.json) | ✅ 通过（3/3 断言，2026-07-16，新定义首次运行；判定见 evals/workspace/2026-07-16-eval-4/outputs/judgment.md；2026-07-14 的旧记录对应收窄前定义，仅供参考） |
| M5 | 第二学科端到端写完整本教材（含阶段 5 自检与交付摘要），验证不过拟合 | [eval 5](evals/evals.json) | ⏳ 待跑 |
| M6 | 工作目录初始化 skill（textbook-init）：规划并创建单本/多教材工作目录 | [eval 6](evals/evals.json) | ⏳ 待跑 |
