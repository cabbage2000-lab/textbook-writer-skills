...
![教材设计示例：教学定位、UbD 五件套大概念表、章节树，标注核心 GATE 与次要 GATE 停点](pic/outline-preview.png)
...
![全书 Bloom 梯度报告示例：各章记忆/理解/应用/分析/评价/创造分布表、全书梯度可视化条形图、三条自动检查规则均通过](pic/gradient-report.png)
...
下图是 `write-textbook` 端到端跑出的《线性代数入门》第 4 章片段——四段式节奏（概念讲解→示范例题→引导练习→独立习题）与每道题的真算验证标签清晰可见：

![教材成品页示例：概念讲解、示范例题、引导练习、独立习题四段式结构，每道题附真算验证标签](pic/chapter-preview.png)
...
| ---------- | ----------------- |
| 从零写一整本教材 | `用 write-textbook 写一本《线性代数入门》教材` |
| 只做教学设计与大纲 | `用 design-textbook-outline 设计一份数据结构教材大纲` |
| 按已有大纲单写一章（也可写带完整例题的深度技术文章） | `用 write-textbook-chapter 按大纲写第三章` |
| 只出几道答案经过验证的题 | `用 generate-textbook-exercises 出几道矩阵乘法的练习题` |
...
```text
用 write-textbook 写一本《线性代数入门》教材
```
...
阶段间交接契约、教材项目落盘布局、`.progress.json` 状态机与重入规则，全部以 [handoff-contract.md](skills/write-textbook/references/handoff-contract.md) 为全项目唯一权威定义。
